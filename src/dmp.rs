...
impl DmpContext {
    pub fn from_bytes(buf: &[u8]) -> Self {
        ...
        Self {
            ...
            xmm,
            debug_control: if has_lbr { read_u64(buf, context::OFFSET_DEBUG_CONTROL) } else { 0 },
            last_branch_to_rip: if has_lbr { read_u64(buf, context::OFFSET_LAST_BRANCH_TO_RIP) } else { 0 },
            last_branch_from_rip: if has_lbr { read_u64(buf, context::OFFSET_LAST_BRANCH_FROM_RIP) } else { 0 },
            last_exception_to_rip: if has_lbr { read_u64(buf, context::OFFSET_LAST_EXCEPTION_TO_RIP) } else { 0 },
            last_exception_from_rip: if has_lbr { read_u64(buf, context::OFFSET_LAST_EXCEPTION_FROM_RIP) } else { 0 },
        }
    }
    ...
    fn extract_triage_crash_info(target: &Target, info: &DmpInfo) -> Option<TriageCrashInfo> {
        ...
        let (thread_id, thread_exit_status) = info
            ...
            .and_then(|thread_snap| {
                ...
                let client_id_layout = target.symbols.find_type_across_modules(dtb, "_CLIENT_ID")?;
                let ut_off =
                    client_id_layout.field_offset("UniqueThread").ok()? as usize;
...
impl DebugBackend for DmpBackend {
    fn initialize_from_target(&mut self, target: &Target) {
        ...
        if let Some(info) = target.phys.dmp_info() {
            self.triage_crash_info =
                Self::extract_triage_crash_info(target, info);
        }
    }
...
mod tests {
    ...
    fn triage_overlapping_blocks_fallback() {
        ...
        let blocks = vec![
            TriageBlock { address: 0x1000, offset: 0x2000, size: 0x3000 },
            TriageBlock { address: 0x2000, offset: 0x5000, size: 0x1000 },
        ];
