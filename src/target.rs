...
impl Target {
    ...
    pub fn with_phys(phys: Arc<PhysMem>) -> Result<Self> {
        ...
        let triage_modules: Option<Vec<ModuleInfo>> = phys
            ...
            .map(|info| info.triage_drivers.iter().map(|d| d.to_module_info()).collect());
    ...
    pub fn reload_guest_with_kernel_base_hint(
        ...
        let (symbol_report, symbol_error) = match guest
    ...
    pub fn current_dtb(&self) -> Dtb {
        ...
        match &self.current_process {
            Some(p) => p.dtb(),
            None => self.context_dtb_override.unwrap_or_else(|| self.kernel_dtb()),
        }
    }
        ...
        let pid = eprocess_layout
            ...
            .and_then(|off| {
                ...
                if off + 8 <= proc_snap.len() {
                    proc_snap[off..off + 8].try_into().ok().map(u64::from_le_bytes)
                } else {
                    None
                }
            })
    ...
    pub fn inspect_object_header(&self, addr: VirtAddr) -> Result<ObjectHeaderDetail> {
        ...
        let (type_object, type_name) =
            match (guest.ntoskrnl.symbol("ObTypeIndexTable"), type_index) {
                (Ok(sym), Some(index)) => {
                    let resolved = mem
    ...
    pub fn startup_message_data(&mut self) -> Result<StartupMessage> {
        let guest = self.guest()?;
        let build_number: u16 = guest.ntoskrnl.symbol("NtBuildNumber")
            .and_then(|s| s.read())
            .unwrap_or(0u16);
        let base_address = guest.ntoskrnl.base_address;
        let loaded_module_list = guest.ntoskrnl.symbol("PsLoadedModuleList")
            .and_then(|s| s.read())
            .unwrap_or(VirtAddr(0));
