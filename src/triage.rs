...
impl TriageDriver {
    pub fn to_module_info(&self) -> crate::guest::ModuleInfo {
        crate::guest::ModuleInfo::new(self.name.clone(), crate::types::VirtAddr(self.base), self.size)
...
pub fn parse_triage(mmap: &[u8]) -> Result<(DmpInfo, Vec<TriageBlock>)> {
    ...
    if mmap.len() < ctx_offset + context::OFFSET_RIP + 8 {
        return Err(Error::InvalidDump("dump too small for context record".into()));
    }
    ...
    let info = DmpInfo {
        ...
        triage_prcb_info: debugger_data.as_ref()
...
fn validate_triage_signature(mmap: &[u8], triage_hdr: &[u8]) {
    ...
    if sig != b"TRGD" {
        diagnostics::eprint_warning("triage dump DGRT integrity signature mismatch — dump may be truncated or corrupt");
    }
}
...
fn parse_debugger_data(mmap: &[u8], triage_hdr: &[u8]) -> Option<DebuggerDataFields> {
    ...
    let min_size =
        std::mem::offset_of!(KdDebuggerData64, offset_prcb_context) + 2;
    ...
    let base = offset;
    let off_prcb_ctx =
        base + std::mem::offset_of!(KdDebuggerData64, offset_prcb_context);
    let off_sz_eprocess =
        base + std::mem::offset_of!(KdDebuggerData64, size_eprocess);
    let off_sz_ethread =
        base + std::mem::offset_of!(KdDebuggerData64, size_ethread);

    let kern_base_raw = read_u64(mmap,
...
fn parse_prcb_info(mmap: &[u8], triage_hdr: &[u8], dd: &DebuggerDataFields) -> Option<TriagePrcbInfo> {
    ...
    let processor_number = if dd.offset_prcb_number > 0
        && (dd.offset_prcb_number as usize) + 2 <= prcb_size
    {
    ...
    let mhz = if dd.offset_prcb_mhz > 0
        && (dd.offset_prcb_mhz as usize) + 4 <= prcb_size
    {
    ...
    let cpu_type = if dd.offset_prcb_cpu_type > 0
        && (dd.offset_prcb_cpu_type as usize) + 2 <= prcb_size
    {
...
mod tests {
    ...
    fn parse_triage_builds_sorted_block_map() {
        let blk_b = TriageBlock { address: 0x2000, offset: 0, size: 0x100 };
        let blk_a = TriageBlock { address: 0x1000, offset: 0, size: 0x80 };
